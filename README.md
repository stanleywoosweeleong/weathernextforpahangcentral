...
locations around Mentakab, central Pahang. Multilingual interface
...
This release migrates Pahang Central from the older forecast-only build onto
...
```
https://stanleywoosweeleong.github.io/weathernextforpahangcentral/
```
...
|---|---|---|---|---|
| Kampung Belengu | 甘榜柏伦古 | 3.436161, 102.294602 | open plain | durian |
| Kampung Ketam | 甘榜吉胆 | 3.54000, 102.36778 | sheltered basin | durian |
| Kampung Gantok | 甘榜甘督 | 3.505847, 102.360318 | sheltered basin | durian |
| Jambu Rias BIGWOOD | 占布里亚斯 大木 | 3.467919, 102.094837 | open plain | durian |
...
The app also seeds a default user display name (**彭亨中部 / Pahang Central**),
which stays editable via **Edit Name** in the app.

This build carries the seed version **`pc-arch1`** (the lowland-architecture
...
```
wnext-weathernextforpahangcentral-202606090000
```
...
- **Storage namespace:** `weathernextforpahangcentral__*` keys in `localStorage`, isolated
  from other WeatherNext regional builds so data never collides.
- **Cloud sync:** Firebase, namespaced under `appId: wnext-ag-v41-weathernextforpahangcentral`.
...
The icon shows a farmer planting under a warm sky. The boot screen background
(cream `#f0dcc1`) is matched to the icon's sky.
